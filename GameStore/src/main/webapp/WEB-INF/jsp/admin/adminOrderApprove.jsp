...
<head>
	...
	<title>ALRO GAMES Approve Orders</title>
...
<div id="main">
  <div class="title">Approve Orders</div>
  ...
  <div id="container">
  	...
  	<table class="numRight">
		...
		<c:forEach var="row" items="${listApproveOrders}" >
			<tr>
				...
				<c:if test="${row.user.level == 'Bronze'}">
					...
					<c:set var="price" scope="request" value="${row.game.bronzePrice}"/>
				</c:if>
				...
				<c:if test="${row.user.level == 'Silver'}">
					...
					<c:set var="price" scope="request" value="${row.game.silverPrice}"/>
				</c:if>
				...
				<c:if test="${row.user.level == 'Gold'}">
					...
					<c:set var="price" scope="request" value="${row.game.goldPrice}"/>
				</c:if>
				...
				<c:if test="${ row.user.balance >= price }">
					<td><a href="goOrderApproval?orderId=${row.id}"><img src="<c:url value='/resources/images/approve.jpg' />" /></a></td>
				</c:if>
				<c:if test="${ row.user.balance < price }">
					<td></td>
				</c:if>
